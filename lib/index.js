...
Tilesplash.prototype.layer = function(name, ___){
  ...
  var self = this;
  this.server.get('/'+name+'/:z/:x/:y.topojson', function(req, res, throwError){
    var render = function(){
    ...
    var parseSql = function(sql, done){
      ...
      var templatePattern = /\!([a-zA-Z_\-0-9]+)\!/g;
      ...
      parseSql(tileOutput, function(parsingError, structuredSql){
        ...
        if (cacher) {
          ...
          self._cache(cacheKey, ttl, function(done){
            ...
            if (topoError) {
              render.error(['topology error', topoError]);
            } else {
              res.send(topology);
            }
          });
